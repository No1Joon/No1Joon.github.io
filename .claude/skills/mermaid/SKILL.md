...
2. **CRITICAL: Mermaid 11.x Syntax Compatibility**:
   - **Never** use double quotes inside special shapes like `A{{"..."}}`, `A[["..."]]`, `A(("..."))` or `A[("...")]`. It crashes the parser.
   - **For labels with parentheses or special chars**, default to using the standard rectangle `A["..."]` which safely handles all inner strings.
   - **Edge labels**: Always wrap edge descriptions in double quotes to escape special characters: `A -->|"텍스트 (기호)"| B`.
   - **Subgraphs**: Keep it clean without quotes using bracket syntax: `subgraph id [Title]`. Do NOT use `subgraph id["Title"]`.
