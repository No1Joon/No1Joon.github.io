...
- OG(`image`)는 별도 생성하지 않고 **그 포스트의 첫 이미지**를 쓴다. 이미지가 하나도 없으면 필드를 생략해 사이트 기본 OG 로 떨어진다.
...
6. 실제 실행 → 잔존 마커 검사 (`🔹 ▸ ▶︎ 📷 ==` 와 `/Users/joon`, `받는 곳:` 이 결과물에 없어야 한다).
7. `bundle exec jekyll build` 로 렌더 확인. 이미지 참조가 전부 `_site` 에 실재하는지 본다.
8. 사용자에게는 편수·카테고리 분포·누락 이미지 건수만 보고. 본문을 다시 붙여 넣지 않는다.
