...
- **배포 전략의 부재**: Canary나 Blue/Green 배포를 직접 스크립트로 구현해야 하며, 롤백 로직도 수동입니다.
...
Harness의 기본 실행 단위입니다. **Stage** 의 묶음으로 구성되며, UI에서 만들거나 YAML로 직접 작성할 수 있습니다. Git 저장소에 YAML로 저장해 코드로 관리하는 것을 권장합니다.
...
Stage 안의 개별 작업 단위입니다. Harness는 100개 이상의 내장 Step 타입을 제공하며, Shell Script Step으로 커스텀 작업도 가능합니다.
...
Harness 플랫폼이 실제 인프라에 명령을 실행하기 위해 사용하는 **에이전트**입니다. Kubernetes 클러스터나 VM에 설치하며, Harness 플랫폼은 Delegate를 통해 kubectl, helm, terraform 등의 명령을 실행합니다.

```mermaid
flowchart TD
    P["Harness Platform (SaaS)"]
    D["Delegate<br/>(클러스터 내부)"]
    K["Kubernetes API"]
    H["Chart 배포"]
    I["이미지 빌드"]
    P -->|"HTTPS (아웃바운드만)"| D
...
반면 단순한 빌드·배포 자동화만 필요하다면 GitHub Actions가 더 가볍고 빠릅니다. Harness는 그 위에 **엔지니어링 플랫폼** 이 필요한 시점에 도입하는 것이 적절합니다.
