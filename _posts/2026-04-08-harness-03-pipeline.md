...
Canary 배포의 핵심은 **소량의 트래픽으로 먼저 검증하고, 문제가 없을 때만 전체로 확장** 하는 거예요.
...
Harness Verify Step은 배포 전후의 메트릭을 비교해요. Prometheus를 Health Source로 연결하는 설정이에요.
...
<div class="callout why">
  ...
  Harness는 배포 전 기간(baseline)과 Canary 배포 후 기간(canary)의 메트릭을 ML 모델로 비교해요. 단순히 임계값을 초과하는지 여부가 아니라, 패턴의 유의미한 변화를 감지해요. sensitivity를 MEDIUM으로 설정하면 작은 노이즈는 무시하고 실질적인 이상만 탐지해요.
</div>
...
CI Stage에서 빌드한 이미지 태그를 CD Stage로 전달하는 방법이에요.
...
GitHub PR 머지 시 자동으로 파이프라인을 실행하는 트리거예요.
