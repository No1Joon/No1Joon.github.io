...
Pipeline: deploy-production
  ...
  │     ├── Step: Static Analysis (ruff, mypy)
  │     ├── Step: Build & Push Image → GAR
  │     └── Step: Update Service Manifest
  │
  ├── Stage 2: Deploy Staging
  │     ├── Step: Rolling Deploy → staging namespace
  │     └── Step: Integration Tests
  │
  └── Stage 3: Deploy Production (Canary)
        ├── StepGroup: Canary Phase
        │     ├── Step: Canary Deploy (20%)
        │     ├── Step: Verify (5분, Prometheus)
        │     └── Step: Canary Delete
        └── Step: Rolling Deploy (100%)
```
