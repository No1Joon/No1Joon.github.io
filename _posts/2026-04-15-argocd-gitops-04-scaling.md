...
description: 수십~수백 개 Application을 선언적으로 생성·관리하는 ApplicationSet과 멀티 클러스터 운영 패턴을 정리해요.
date: 2026-04-15
order: 4
category: CI/CD
...
Application을 한두 개 쓸 때는 앞 글의 방식으로 충분해요. 하지만 서비스가 수십 개로 늘어나고, dev·stg·prod 3벌씩 구성하면 금방 **100개가 넘는 Application 리소스**를 수작업으로 관리하게 돼요. 여기서 등장하는 게 `ApplicationSet`이에요. "Application을 만드는 템플릿"을 선언적으로 정의하면 generator가 알아서 필요한 수만큼 찍어내요.

## 왜 ApplicationSet인가

간단한 비교예요. 10개 서비스를 3개 환경에 배포하면 Application이 30개 필요한데, 이걸 YAML 30장으로 관리하면 변경이 생길 때마다 지옥이에요.
...
ApplicationSet의 심장은 **Generator**예요. "몇 개의 Application을 어떤 파라미터로 만들지"를 결정하는 공급원이에요. 대표적인 타입만 추려봤어요.
...
ApplicationSet 하나로 각 디렉토리를 자동 감지해서 Application을 생성해요.
...
새 서비스를 추가하려면 **디렉토리만 만들면** 돼요. ApplicationSet controller가 감지해서 Application을 자동 생성해요.

## Matrix Generator — 환경 × 서비스 조합

"서비스 × 클러스터" 조합을 전부 만들어야 할 때 Matrix가 유용해요.
...
3개 서비스 × 3개 클러스터 = 9개 Application이 템플릿 하나로 생성돼요. **서비스가 10개로 늘어도 템플릿은 그대로**예요.

## 멀티 클러스터 등록

ApplicationSet이 여러 클러스터에 배포하려면 먼저 ArgoCD에 그 클러스터들이 등록돼 있어야 해요. 클러스터는 **Secret으로 표현**돼요.
...
라벨이 중요한 이유는 **Cluster Generator에서 필터링**에 쓰이기 때문이에요.
...
이렇게 하면 "prod 라벨이 붙은 클러스터들에만 배포"가 선언 한 줄로 해결돼요.

## Hub-and-Spoke vs Stand-Alone

멀티 클러스터 환경에서 ArgoCD를 어떻게 배치할지는 두 가지 전략이 있어요.
...
실무에서는 **Hub-and-Spoke가 기본**이고, 보안·규제로 클러스터 간 통신이 막히거나 대륙이 다른 경우에만 Stand-Alone으로 쪼개요.

## AppProject — 멀티 팀 RBAC

ApplicationSet으로 Application을 대량 생성하면 이제 **"누가 어떤 Application을 건드릴 수 있는가"**가 문제가 돼요. 기본 `default` Project는 모든 권한이 열려있어서, 프로덕션 환경에서는 팀·환경별 Project를 반드시 만들어요.
...
대규모 환경에서 자주 부딪히는 이슈와 대응법이에요.

<div class="callout why">
  <div class="callout-title">Application 수 늘어나면 가장 먼저 터지는 건 repo-server 메모리</div>
  Application 500개 환경에서 Helm 차트를 주로 쓰면 repo-server가 OOMKilled로 반복 재시작되는 걸 자주 봐요. `--parallelismLimit` 을 낮추고 repo-server replicas를 늘리거나, 아예 <b>Helm render 결과를 미리 Git에 커밋해두는 rendered-manifest 패턴</b>으로 전환하면 repo-server 부하를 극적으로 줄일 수 있어요.
</div>
...
4편에 걸쳐 GitOps 철학에서 시작해 ArgoCD 아키텍처, Application 단위의 동기화 전략, 그리고 ApplicationSet·멀티 클러스터 운영까지 다뤘어요. 핵심 메시지를 한 줄로 요약하면:
...
다음 시리즈에서는 ArgoCD가 배포하는 **컨테이너 자체를 어떻게 효율적으로 만들지** — Docker 이미지 설계부터 깊이 있게 파고들어봐요.
