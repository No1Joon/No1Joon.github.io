...
description: Python 서비스의 Lint·Test·Build·Deploy를 하나의 워크플로우에 담고, Job 간 의존·조건·출력 전달을 정리해요.
date: 2026-04-15
order: 2
category: CI/CD
...
GitHub Actions로 파이프라인을 만들 때 다음 세 가지를 지키면 복잡도가 급격히 낮아져요.

1. **관심사 분리**: 빌드·테스트·배포를 Job으로 분리하고 `needs` 로 묶어요.
2. **조건부 실행**: 같은 워크플로우에서 PR은 테스트만, push는 배포까지 가도록 `if` 로 분기해요.
3. **환경 격리**: Staging·Production은 GitHub Environment로 분리하고 Secret도 각각 저장해요.
...
Lint와 Test는 병렬로 돌려요. `needs` 없이 선언하면 동시에 시작해요.
...
<div class="callout why">
  ...
  Environment에는 Required Reviewers·Wait Timer·Deploy Branch 제한 같은 보호 규칙을 붙일 수 있어요. Production Environment에 Required Reviewers를 걸면 Job이 대기 상태로 멈추고, 지정된 리뷰어가 승인해야 실행돼요. 코드 추가 없이 승인 게이트를 만들 수 있어요.
</div>

## 조건부 실행 패턴

`if` 표현식으로 Job·Step의 실행 여부를 결정해요. 자주 쓰는 패턴이에요.
...
같은 브랜치에서 연속으로 push가 들어오면 이전 실행을 취소하고 최신만 실행해요.
...
Production 배포에는 `cancel-in-progress: false` 로 바꿔 취소되지 않게 해요 — 중간에 끊기면 인프라가 이상한 상태에 남을 수 있어요.
...
다음 글에서는 정적 키 없이 AWS·GCP에 배포하는 OIDC 인증 설정과 Secret 관리 원칙을 다뤄요.
