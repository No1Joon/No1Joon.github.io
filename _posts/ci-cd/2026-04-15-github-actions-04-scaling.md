...
description: Reusable Workflow·Composite Action·Matrix·Cache·Self-hosted Runner로 대규모 워크플로우를 재사용하고 빠르게 만드는 방법이에요.
date: 2026-04-15
order: 4
category: CI/CD
...
여러 저장소·워크플로우가 생기면 중복이 폭발적으로 늘어나요. GitHub Actions에서는 세 가지 축으로 확장성과 속도를 확보해요.
...
피호출 워크플로우는 `on: workflow_call` 트리거를 선언하고, 호출 측은 `uses:` 로 파일 경로를 참조해요.
...
중앙 저장소의 태그(`@v1`)를 고정해 호출 측이 영향받지 않도록 관리해요.
...
Step 레벨에서 반복되는 동작은 Composite, 파이프라인 전체를 공유하려면 Reusable Workflow를 써요.

## Matrix Strategy

여러 버전·OS 조합을 한 선언으로 병렬 실행해요.
...
의존성 다운로드·빌드 산출물을 Run 간에 보존해요. 키 설계가 성능을 좌우해요.
...
<div class="callout why">
  ...
  <code>key</code> 가 정확히 일치하는 캐시가 있으면 그걸 복원하고, 없으면 새로 저장해요. <code>restore-keys</code> 는 정확한 매치가 없을 때 prefix 매칭으로 이전 캐시를 복원하지만 <strong>새로 저장하지는 않아요</strong>. 의존성 파일이 바뀌어도 가까운 이전 캐시를 부분 활용해 설치 시간을 줄이는 게 목적이에요.
</div>
...
`type=gha` 는 GitHub Actions 캐시 API를 레이어 저장소로 써요. 별도 Registry 없이 동작해요.

### 캐시 제한

- 저장소당 총 10GB. 초과분은 LRU로 자동 삭제.
- 7일간 미사용 캐시는 자동 삭제.
- 브랜치별로 격리 — PR은 base 브랜치 캐시를 읽을 수 있지만 PR에서 저장한 캐시는 base에서 못 읽어요.

## Artifact 전달

Artifact는 Job 간·워크플로우 간 파일 전달용이에요. 캐시와 목적이 달라요.
...
라벨을 여러 개 붙이면 워크플로우에서 정밀하게 지정할 수 있어요.
...
<div class="callout why">
  ...
  퍼블릭 저장소의 PR은 누구나 열 수 있어요. Self-hosted Runner에서 PR의 코드를 그대로 실행하면 사내망 공격이 가능해요. 반드시 <strong>프라이빗 리포 전용</strong>으로 두거나, 퍼블릭 리포라면 조직 멤버 PR만 실행되도록 조직 설정에서 제한해요.
</div>

### Ephemeral Runner

매 Job마다 Runner를 새로 띄워 격리하는 방식이에요. actions-runner-controller(ARC)로 Kubernetes 위에서 운영하면 자동 스케일링과 자동 폐기가 동시에 돼요.
...
GitHub Actions 시리즈는 여기서 마무리해요. 구조·파이프라인·보안·확장 네 축이 준비되면 대부분의 팀이 쓰는 CI/CD 요구를 커버할 수 있어요.
