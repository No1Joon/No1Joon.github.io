...
가장 보편적으로 사용되는 오브젝트입니다. 업데이트 시 **RollingUpdate** 전략을 사용하여 트래픽 단절 없이 새 버전을 배포할 수 있습니다. `maxSurge`와 `maxUnavailable` 옵션을 통해 배포 중 가용 자원 범위를 세밀하게 조정해요.
...
- **Liveness**: 컨테이너가 살아있는지 확인 (실패 시 재시작)
- **Readiness**: 트래픽을 받을 준비가 되었는지 확인 (실패 시 서비스 제외)
- **Startup**: 앱 초기화 완료 여부 확인 (완료 전까지 위 두 검사 유예)
...
다음 글에서는 이 Pod들이 서로 연결되고 외부로 노출되는 방식인 **네트워킹** 구조를 정리해요.
