...
- `Dockerfile` — 배포용 컨테이너 빌드 (uv sync + uvicorn).
- `.env.example` — 필요한 환경 변수 템플릿. 실제 값(`.env`, `prod.env.yaml`)은 `.gitignore` 로 제외.
...
- `uv run pytest` — 테스트 실행 (`pyproject.toml` 의 `testpaths = ["tests"]`; 현재 `tests/` 미작성).
