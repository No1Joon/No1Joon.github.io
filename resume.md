...
**React (웹 · React Native/Expo)** · **TypeScript** · **Vue 3 / Vuetify** · Redux · Redux Toolkit · Zustand · SPA 아키텍처 · 공통 컴포넌트 표준화 · config-driven UI · i18n · 크로스플랫폼(iOS/Android)

### Backend / Infra

**Node.js / NestJS** · Python · MongoDB · **Temporal** · Kafka · Docker / Cog · Kubernetes · GCP (Cloud Run, Vertex AI) · AWS S3
...
**코드 리뷰 · 기술 리딩** · TypeScript 타입/데이터 모델링 · Jest · ESLint / Prettier · CI/CD · 제3자 공인 시험 인증 대응
...
- **Config-driven 대시보드** — 백엔드가 내려주는 JSON config(그리드·컴포넌트·MathJS 바인딩)를 프론트가 동적 렌더링해 비개발자(BA/PM)가 코드 개입 없이 매장별 화면을 구성하게 함, 패션 리테일 매장 첫 상용 도입(PoC) 성사
- 영어·한국어·일본어·바하사 **4개 언어 i18n 시스템 설계** 및 레거시 코드 리팩토링 — 3개국 대상 서비스 기반 마련
...
- **NestJS / TypeScript 백엔드** — 분석 데이터 export API, Role/Permission·멀티테넌시 설계, 잡보드 속성 서버 검증, 코인 카운트·완료율 계산 오류 수정으로 태거 보상 정합성 확보
- 검출·추적 **AI 모델을 Cog 컨테이너로 패키징** — SAM2MOT 프로세서 분리, 입력 해상도(1024→512)·score threshold·skip frame 튜닝으로 정확도 유지하며 처리 비용 절감
...
- 데이터바우처 사업 **수요기업 13곳**(2021, 2023–2025)의 데이터 가공·품질검사 파이프라인 직접 구현 — **EgoBlur·랜드마크 기반 얼굴 비식별화**, Whisper 실시간 STT, 설문 데이터 정제·분석, 이미지–CSV 정합성 검사, 포맷·해상도 검증
- 수요기업별 **협약 체결 실무**부터 데이터 항목정의서·코드정의서·인수기준서 등 규격 산출물 작성과 **중간·최종 감리 통과**까지 담당, 반복 작업을 가이드라인으로 표준화해 Ops팀 이관
...
- **QC 자동화 로봇 4종** 구축 — Gradio 기반 자동 어노테이션 툴(세션 상태 전환으로 다중 사용자 지원), Coin Decision Robot, Source-of-Truth QC Robot, 광고 이미지 **12만 장 자동 QC**
